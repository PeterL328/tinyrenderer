...
namespace TinyGL {
    ...
    void
    drawTriangle(MainWindow *window, Model *model, const Shader &shader, const Geometry::Vec3f *pts,
                 const Geometry::Vec2f *texture_pts,
                 const Geometry::Vec3f *normal_pts, float *depth_buffer, const Geometry::Vec3f &camera_source);

    void
    drawFace(MainWindow *window, const Geometry::Matrix &p_m, const Geometry::Matrix &v_p, const Geometry::Matrix &c_m,
             Model *model,
             const Shader &shader, const Geometry::Vec3f &camera_source, const int width, const int height);

    Geometry::Matrix
    camera_matrix(const Geometry::Vec3f &eye, const Geometry::Vec3f &center, const Geometry::Vec3f &up);
