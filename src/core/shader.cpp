...
Shader::Shader(const float ambient_lighting, const float diffuse_reflectivity, const float specular_reflectivity, const float specular_intensity, const Vec3f &light_dir):
...
TGAColor Shader::get_shaded_color(const TGAColor &color, const Vec3f &normal, const Vec3f &viewing_location, const Vec3f &current_pt) const {
    TGAColor shaded_color{};
    Vec3f norm_point_to_light = (light_dir_ * -1).normalize();
    float light_n_normal = std::max(0.f, normal * norm_point_to_light);
    Vec3f norm_point_to_camera = (viewing_location - current_pt).normalize();
    Vec3f h = (norm_point_to_camera + norm_point_to_light).normalize();
